...
module.exports = class BeTunnelClient {
  ...
  write(socket, data, callback = function(err){ err && console.log('WRITE SOCKET ERR : ' + err); }) {
  ...
  writeSelfSocket(data) {
    ...
    self.write(self.socket, data, function(err) {
      err && console.log('WRITE TUNNEL ERROR : ' + err);
    });
  }
          ...
          socket.connect(self.config.backend_port, self.config.backend_host, function() {
            ...
            socket.on('data', function(buffer){
              ...
              while (endOffset < buffer.length) {
                endOffset += buffer.length > max ? max : buffer.length;
                let b = buffer.slice(offset, endOffset);
                let f = new Frame(Frame.Type.data, frame.clientId, frame.userId, b);
                self.writeSelfSocket(f.buffer);
