...
function parseBufferToBufferArray(buffer) {
    ...
    console.log( i + ': parseBufferToBufferArray: ' + buffer);

    if (bufferCache.length) {
        // 上次有缓存，拼接后处理
        console.log(i + ': 合并多个数据包，方便后续解析');
    ...
    if (buffer.readUInt16BE(0) !== header) {
        console.log(i + ': ❌数据头错误 : parseBufferToBufferArray(' + buffer.length + ') : ', buffer);
        bufferCache = [];
        return [];
    }
    ...
    if (buffer.length < length) {
        // 缓存不足够长的包
        console.log(i + ': 数据包不够长 目标长度：' + length + '，实际长度：' + buffer.length);
        bufferCache.push[buffer];
        return [];
    }
